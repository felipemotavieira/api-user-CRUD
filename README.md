...
Esse projeto consiste na primeira API que foi desenvolvida nos estudos de back-end no curso da Kenzie Academy Brasil. Nele, existem rotas de criação de usuário, login, listagem, alteração e deleção de usuários. As libs utilizadas foram express, dotenv, nodemon, sucrase, jest. Para segurança segurança foram utilizadas bcrypt, JWT, além da UUID. Segue abaixo as instruções para realização da atividade disponibilizadas pela Kenzie. 
